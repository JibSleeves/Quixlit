...
from reworkd_platform.web.api.agent.agent_service.llama_agent_service import (
    LlamaAgentService,
)
...
def get_agent_service(
    ...
    def func(
        ...
        # Use LlamaAgentService if llama mode is enabled
        if settings.ff_llama_mode_enabled:
            # Check if model is one of the llama models
            model_str = str(llm_model or run.model_settings.model)
            if model_str.startswith("llama-"):
                return LlamaAgentService(
                    run.model_settings,
                    token_service,
                    callbacks=None,
                    user=user,
